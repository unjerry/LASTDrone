...
void DrawPolygon(const b2Vec2 *vertices, int vertexCount, b2HexColor color, void *context)
{
    ...
    // }
    // printf("\n");
    Rend->DrawSprite(ResourceManager::GetTexture("background"), glm::vec3(vertices[0].x / 30, vertices[0].y / 30, 0.0f), glm::vec2((vertices[2].x - vertices[0].x) / 30, (vertices[2].y - vertices[0].y) / 30), 0.0f);
}
